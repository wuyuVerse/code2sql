...
# 用于重新分析被标记为 <NO SQL GENERATE> 的记录的优化提示词
REANALYSIS_PROMPT = """
请基于以下分析要求，直接输出GORM代码对应的SQL语句JSON格式结果：

**首要判断：SQL生成有效性**
请判断给定的ORM代码不生成sql是否正确，如果能生成sql，需要给出原因：
...
[
  {{
    "should_generate_sql": "是否不应该生成sql,填写true 或者 false",
    "reason": "原因描述;"
  }}
]
