CODE_ORM_MYSQL_SQL_EXTRACT = \
    ...
    "   - 如果代码中没有传入任何筛选条件，即 `filter` 为空，没有caller，或者 `Where` 条件没有被动态构建（没有筛选条件），那么SQL应该为 `WHERE ?`，即没有任何过滤条件的查询。这种情况下返回：`<LACK INFORMATION>: 缺少筛选条件 | WHERE ?`\n" \
...
CODE_ORM_MYSQL_SQL_VERIFY = \
    ...
    "4. **边界条件验证规则**：\n" \
    "   - 对于没有筛选条件的情况，生成的SQL的WHERE条件应该为 `WHERE ?`，即没有任何WHERE条件。\n" \
    "   - 如果分析结果包含`<LACK INFORMATION>`并且描述了缺少筛选条件，那么SQL的WHERE条件应为 `WHERE ?`。\n" \
...
CODE_ORM_MYSQL_SQL_FORMAT = \
    ...
    "**无SQL生成标记处理**：\n" \
    "   - 如果分析结果包含`<LACK INFORMATION>:`标记并且描述缺少筛选条件，说明没有传入筛选条件。\n" \
    "   - 在这种情况下，SQL的where条件应为 `WHERE ?`，即没有筛选条件。\n" \
    "2. **边界条件格式化**：\n" \
