...
try:
    ...
    from config.data_processing.validation.redundant_sql_validation_prompt import (
...
class RedundantSQLValidator:
    ...
    def __init__(self, output_dir: str = ".", llm_server: str = None):
        """
        初始化验证器
    ...
    async def _validate_redundant_candidate(self, candidate: Dict[str, Any]) -> Tuple[str, List[Dict]]:
        ...
        for sql_record in redundant_sqls:  # 处理所有SQL，移除[:3]限制
            ...
            from config.data_processing.workflow.workflow_config import get_workflow_config
            workflow_config = get_workflow_config()
            max_tokens = workflow_config.get_max_tokens("validation", "redundant_sql_validator")
            ...
            response = await self.llm_client.call_async_with_format_validation(
                ...
                module="validation", component="redundant_sql_validator"
            )
    ...
    async def _validate_new_fingerprint_candidate(self, candidate: Dict[str, Any]) -> Tuple[str, List[Dict]]:
        ...
        for sql_record in new_sqls:  # 处理所有SQL，移除[:3]限制
            ...
            response = await self.llm_client.call_async_with_format_validation(
                ...
                temperature=0.0,
                module="validation",
                component="redundant_sql_validator"
            )
