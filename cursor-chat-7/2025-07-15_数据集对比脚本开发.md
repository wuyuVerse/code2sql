...
用户要求基于现有的 `run_comparison.py` 脚本，创建一个新的数据集对比脚本，用于直接比对两个数据集（基准数据集和生成数据集），而不需要模型推理，同时要求生成的JSON报告适合在网站上渲染。

## 实现方案

### 1. 核心功能设计

创建了 `run_dataset_comparison.py` 脚本，主要特点：
...
该脚本可以作为模型评估流程的重要组成部分，用于验证生成数据的质量和准确性。 